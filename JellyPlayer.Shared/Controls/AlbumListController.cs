...
public class AlbumListController : IDisposable
{
    ...
    public AlbumListController(IJellyPlayerApiService jellyPlayerApiService, IConfigurationService configurationService, IPlayerService playerService, IFileService fileService)
    {
    ...
    private async Task<List<Album>> GetNextPatchOfAlbumsAsync()
    {
        ...
        {
            queries.Add(_jellyPlayerApiService.GetArtistsAndAlbumsAsync(StartIndex, Count));
            StartIndex += Count;
        }
    ...
    private protected virtual void AlbumListChanged(AlbumListStateArgs e)
    {
    ...
    {
        var collectionId = _jellyPlayerApiService.GetCollectionId();
